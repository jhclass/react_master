...
import {getMovies, IMovie,getMoviesList,IMovieList, getTvList,ITvList} from '../api';
...
function Tv() {
    ...
    const list = useQuery<ITvList>('list',getTvList);
    const [index,setIndex] = useState(0);
    const [leaving,setLeaving] = useState(false);
    const bigMovieMatch = useRouteMatch<{tvId:string}>("/tv/:tvId");
    //console.log('첫번쨰 무비 이름',list.data?.results[0].name)
    const clickMovie = bigMovieMatch?.params.tvId && 
    list.data?.results.find((tv)=>tv.id+""===bigMovieMatch.params.tvId);
    ...
    console.log('tva',list.data?.results);
    console.log('tv타이틀',list.data?.results[0].title);
    ...
    return <Wrapper>{list.isLoading?
        ...
        <>
            ...
            >
                <Title>{list.data?.results[0].name}</Title>
            ...
            <Slider>
                ...
                <AnimatePresence initial={false} onExitComplete={toggleLeaving}>
                    <Row initial={{ x: width + 10 }} animate={{ x: 0 }} exit={{ x: -width - 10 }} transition={{type:"tween",duration:1}} key={index}>
                        {list.data?.results.slice(offset*index,offset*index+offset).map((datas,i)=><Boxes id={datas.id} title={datas.name} description={datas.overview} index={i} imgPath={datas.backdrop_path} key={i} category="tv"/>)}
                    </Row>
                </AnimatePresence>
            </Slider>
            <AnimatePresence>
